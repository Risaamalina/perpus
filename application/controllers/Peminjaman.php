...
class Peminjaman extends CI_Controller{ function __construct(){
     parent::__construct(); 
     // cek login 
	if($this->session->userdata('status') != "login"){ $alert=$this->session->set_flashdata('alert', 'Anda belum Login'); 
	redirect(base_url()); 
         }
       } 
	...
	//one to many
     public function tambah_pinjam($id){ 
     	if($this->session->userdata('status') != "login"){$alert=$this->session->set_flashdata('alert','Anda belum Login'); 
        	...
        	$o = $this->M_perpus->edit_data(array('id_buku'=>$id,'id_anggota' => $this->session->userdata('id_agt')),'transaksi')->num_rows(); 
        	if($o>0) { 
